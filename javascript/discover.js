...
d.addEventListener("DOMContentLoaded", function(){
	...
	var toggleModal = function(e){
		...
		if(e.target.id == "filter-modalContainer" || modalContainer.contains(e.target)){
	...
	contentBoxes.forEach(content => {
		...
		content.addEventListener('mouseenter', function(){
			...
			if (contentImage) {
				content.querySelector('.content-type-overlay').style.backgroundColor = "#00000070";
		...
		content.addEventListener('mouseleave', function(){
			...
			if (contentImage) {
				content.querySelector('.content-type-overlay').style.backgroundColor = "transparent";
	...
	filterTypes.forEach(type => {
		...
		type.addEventListener('click', function(){			
			...
			(this.classList.contains('active-type')) ? this.classList.remove("active-type") : this.classList.add("active-type");
